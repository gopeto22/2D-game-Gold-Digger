let backGroundTutorial

let pressEscToGoBack
...
let w, a, s, d
let wSprite, aSprite, sSprite, dSprite

let cursors
let upSprite, leftSprite, downSprite, rightSprite
...
//224*144
        ...
        contentT=[
            ...
            "Играч 1 (Брок) използва W, A, D, за да се движи из картата, а Играч 2 (Еди) използва стрелките.",
            "Играчите трябва да внимават да не попаднат на шиповете. ",
            "В противен случай ще се върнат в началната си позиция.",
            "Печели този, който събре повече злато."
        ];
        textT = game.add.text(
            this.world.centerX, 130, '', { font: "25px Arial", fontWeight: "bold", fill: "rgba(204, 122, 0)", align:"left", backgroundColor: "rgba(0, 0, 0, 0.5)"}
        );
        textT.anchor.x = 0.5;
        text.lineSpacing = -4
        nextLineT();
        ...
        pressEscToGoBack = game.add.text(
            150, game.world.height - 80, 'Назад(ESC)', {fill: 'black'}
        );
        pressEscToGoBack.anchor.x= 0.5
        pressEscToGoBack.anchor.y = 0.5
        color1GoBack();
        ...
        pressEnterToCon = game.add.text(
            game.world.width - 150 , game.world.height - 80, '  Старт(ENTER)', {fill:'black'}
        );
...
const moveBackgroundTutorial = function () {
    game.time.events.add(Phaser.Timer.SECOND * 0.001, moveBackgroundTutorial, this);
    backGroundTutorial.tilePosition.x-=2;
}

const nextLineT = function () {
    game.time.events.add(Phaser.Timer.SECOND * 25, nextLineT, this);
    if (lineIndexT === contentT.length)
    {
        return;
    }

    lineT = contentT[lineIndexT].split(' ');

    wordIndexT = 0;

    game.time.events.repeat(wordDelayT, lineT.length, function(){
        textT.text = textT.text.concat(lineT[wordIndexT] + " ");
        wordIndexT++;

        if (wordIndexT === lineT.length)
        {
            textT.text = textT.text.concat("\n");
            game.time.events.add(lineDelayT, nextLineT, this);
        }
    }, this);
...
const controlsAnimation= function() {
    game.time.events.add(Phaser.Timer.SECOND * 0.001 , controlsAnimation, this);
    if(w.isDown){
        wSprite.loadTexture('w1')
        ...
        wSprite.loadTexture('w')
        
    }
    if(a.isDown){
        aSprite.loadTexture('a1')
        player1.body.velocity.x = -150
        player1.animations.play('walkLeft')
    }
    else if(d.isDown){
        aSprite.loadTexture('a')
        dSprite.loadTexture('d1')
        player1.body.velocity.x = 150
        player1.animations.play('walkRight')    
    }
    else {
        aSprite.loadTexture('a')
        dSprite.loadTexture('d')
        player1.body.velocity.x = 0
        player1.animations.stop()
        player1.frame=0
    }
    if(s.isDown){
        sSprite.loadTexture('s1')
    }
    else{
        sSprite.loadTexture('s')
    }



    if(cursors.up.isDown){
        upSprite.loadTexture('up1')
        ...
        upSprite.loadTexture('up')
    }
    if(cursors.left.isDown){
        leftSprite.loadTexture('left1')
        player2.body.velocity.x=-150
        player2.animations.play('walkLeft')
    }
    else if(cursors.right.isDown) {
        leftSprite.loadTexture('left')
        rightSprite.loadTexture('right1')
        player2.body.velocity.x=150
        player2.animations.play('walkRight')
    }
    else{
        leftSprite.loadTexture('left')
        rightSprite.loadTexture('right')
        player2.body.velocity.x=0
        player2.animations.stop()
        player2.frame=0
    }
    if(cursors.down.isDown){
        downSprite.loadTexture('down1')
    }
    else{
        downSprite.loadTexture('down')
    }
}

const color1GoBack = function () {
    game.time.events.add(Phaser.Timer.SECOND * timer, color2GoBack, this);
    pressEscToGoBack.addColor("#ffdf00", 6)
    
}

const color2GoBack = function () {
    game.time.events.add(Phaser.Timer.SECOND * timer, color3GoBack, this);
    pressEscToGoBack.addColor("#ff0000", 6).addColor("black", 9)

}

const color3GoBack = function () {
    game.time.events.add(Phaser.Timer.SECOND * timer, color4GoBack, this);
    pressEscToGoBack.addColor("#11ff00", 6)
}

const color4GoBack = function () {
    game.time.events.add(Phaser.Timer.SECOND * timer, color5GoBack, this);
    pressEscToGoBack.addColor("#0091ff", 6)

}

const color5GoBack = function () {
    game.time.events.add(Phaser.Timer.SECOND * timer, color6GoBack, this);
    pressEscToGoBack.addColor("#bfbfbf", 6)

}

const color6GoBack = function () {
    game.time.events.add(Phaser.Timer.SECOND * timer, color1GoBack, this);
    pressEscToGoBack.addColor("#cc00cc", 6)
